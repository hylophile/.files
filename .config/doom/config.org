...
;; (desktop-save-mode 1)
;; always restore desktop. for some reason desktop-save-mode doesn't work.
(add-hook 'window-setup-hook #'desktop-revert 100)
...
(defun meow-setup ()
  ...
  (define-key doom-leader-map (kbd "m") (meow--parse-def "C-c l"))

  (setq meow-ignore-keymap (make-keymap))
  (meow-define-state ignore
    "meow ignore state"
    :lighter " [I]"
    :keymap meow-ignore-keymap)

  (setq meow-cursor-type-ignore 'hollow)

  (setq my/meow-ignored-modes
        '(notmuch-show-mode
          notmuch-search-mode
          notmuch-tree-mode
          notmuch-hello-mode))

  (dolist (ignored-mode my/meow-ignored-modes)
...
(require 'meow)
(meow-setup)
(define-key meow-motion-state-keymap (kbd "SPC") #'doom/leader)
(meow-global-mode 1)
...
;; (setq meow-keypad-literal-prefix (kbd "SPC"))
...
;; (map! :map doom-leader-map "h" 'help-map)

(define-key doom-leader-map (kbd "h") help-map)
;;   "w" #'meow-keypad-start)
;; (setq doom-localleader-alt-key nil)
;; (map! :map meow-leader-keymap
;;   "l" #'meow-keypad-start)
...
(custom-set-faces!
  '(org-hide :foreground "rgba(0,0,0,0)")
  `(fringe
