...
Whenever we're not in a git directory, we open the =magit-status= of our dotfiles repo.
#+begin_src emacs-lisp
...
(map! :after magit :leader "g g" #'hy/magit-status-with-dotfiles-fallback)
#+end_src

Then, this advice takes over and sets the git environment appropriately:
#+begin_src emacs-lisp
...
#+end_src
With this, we can stage the current (untracked) dotfile with =SPC g S=.
#+begin_src emacs-lisp
