...
(defun abbreviate-directory-path (path)
  "Turn `~/code/test/project` into `~/c/t/project`"
...
(defun hy/nice-name (&rest _)
  (let ((dir (abbreviate-directory-path (abbreviate-file-name (or (projectile-project-root) default-directory)))))
    (set-persp-parameter 'dir dir)))

(advice-add #'+workspace-switch :before #'hy/nice-name)
